...
-- For security prevent arbitrary code execution
...
if not (vim.uv or vim.loop).fs_stat(lazypath) then
  ...
  local secure_commit_hash = '85c7ff3'
  local out = vim.fn.system { 'git', 'clone', '--filter=blob:none', lazyrepo, lazypath }
  if vim.v.shell_error ~= 0 then
    error('Error cloning lazy.nvim:\n' .. out)
  end

  local checkout_out = vim.fn.system{ 'git', '-C', lazypath, 'checkout', secure_commit_hash }
  if vim.v.shell_error ~= 0 then
    error('Error checking out commit for lazy.nvim:\n' .. checkout_out)
  end
...
-- SECURITY: Pinned to a specific commit since tpope doesn't use tags
  {
    'tpope/vim-sleuth',
  ...
  {
    'tpope/vim-fugitive',
    -- SECURITY: Pinned to a specific commit to prevent upstream supply chain attacks
...
{ -- Highlight, edit, and navigate code
    ...
    config = function()
      ...
      -- Manage your languages through the new install API
      ts.install({ 'bash', 'c', 'diff', 'html', 'lua', 'luadoc', 'markdown', 'markdown_inline', 'query', 'vim', 'vimdoc', 'java', 'cmake' })

      -- Attach native treesitter highlighting to your buffers
      vim.api.nvim_create_autocmd('FileType', {
        pattern = { 'bash', 'c', 'diff', 'html', 'lua', 'markdown', 'query', 'vim', 'java', 'cmake' },
        callback = function()
